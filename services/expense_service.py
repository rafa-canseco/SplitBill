...
def update_session_total(session_id: int, new_expenses_total: Decimal):
    session_response = (
        supabase.table("sessions").select("total_spent").eq("id", session_id).execute()
    )
    ...
    supabase.table("sessions").update({"total_spent": float(new_total_spent)}).eq(
        "id", session_id
    ).execute()
...
def update_user_total(session_id: int, user_id: int, amount: Decimal, timestamp: str):
    ...
    if current_user_data.data:
        ...
        supabase.table("sessions_users").update(
            {"total_spent": float(new_total_spent), "last_update": timestamp}
...
def create_multiple_expenses(session_id: int, user_expenses: List[UserExpense]):
    ...
    try:
        expenses_response = create_expenses(
            session_id, user_expenses, current_timestamp
        )

        new_expenses_total = Decimal(
            sum(Decimal(str(expense.amount)) for expense in user_expenses)
        )
