...
def create_user(privy_id: str, name: str, email: str, walletAddress: str) -> UserResponse:
        new_user_data = {
            "privy_id": privy_id,
            "name": name,
            "email": email,
        ...
        user_response = supabase.table("users").select("*").eq("id", created_user_id).execute()
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Error creating UserResponse: {str(e)}")
