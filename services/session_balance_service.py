...
def checkout_session(session_id: int) -> Dict:
    try:
        ...
        users_info = (
            ...
            .select("id,name,email,walletAddress")
            .in_("id", user_ids)
            .execute()
        )
        ...
        users_spent = {
            user["user_id"]: Decimal(str(user["total_spent"]))
        ...
        balances = {
            user_id: spent - fair_share for user_id, spent in users_spent.items()
        }
        print(balances)
        ...
        supabase.table("sessions").update({"state": "waiting_payment"}).eq(
            "id", session_id
        ).execute()
        ...
        return {
            ...
            "payers": {
                str(k): {
                    "amount": float(v),
                    "name": users_info_dict[k]["name"],
                    "email": users_info_dict[k]["email"],
                    "walletAddress": users_info_dict[k]["walletAddress"],
                }
